...
## Stage 4 增量扩展（2026-02-27）
1. 目标
- 基于 Task-006 主线继续扩展错题复盘体验，补齐“错题本”与按时间优先复盘能力。
2. 扩展范围
- 错题本持久化：为每单元记录错题列表与最近更新时间。
- 错题本页面：支持跨单元查看错题、查看正确答案与解释、进入仅做错题模式。
- 复盘效率：按最近错题时间倒序展示，支持清空单元错题。
- 地图联动：在单元卡片展示“最近错题时间”，便于快速识别近期薄弱项。
- 测试补强：新增 `progressStore` 状态测试与持久化时间戳测试。
3. 验收
- 地图页可进入错题本，错题本可按“最近更新”排序。
- 地图页单元卡片可见“最近错题时间”（无记录显示 `—`）。
- 单元满分后错题记录自动清空；手动清空可即时生效并持久化。
- `npm run lint/test/build` 全绿。
...
- Commit：
  ...
  - `dc12932`（Sub-3）
  - `65f06b8`（Sub-4）
  - `d2b2214`（traceability）
  - `7047f4b`（Stage 4 扩展：错题本持久化/页面/排序/清空/测试）
  - `01333ea`（Stage 4 扩展：地图页展示最近错题时间）
- PR：[#37](https://github.com/gui16789/zpdAcademy/pull/37)
